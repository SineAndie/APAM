...
Notes on how to modify inputs and model formulation are detailed in the following section.
```{r APAM}
library(APAM)
```
...
plots$index_fit1
```
```{r surveyfit2, fig.width=10, fig.height=6,dpi=200}
...
The natural mortality assumption, including M split for years 1989-1996, can be changed 
...
Additionally, can turn process errors on/off through the make.map function and make.parameters.
...
fit2$obj$fn()
```
The parameter map can be manually changed via setmap (still needs a bit of work/cleanup but in progress!)
```{r new parms}
##default formulation: 
#setmap <- list(
    #   meanF= c("5","6"),
...
To get profile likelihoods and local infuence diagnostics from Perreault & Cadiagan 2021 (in progress).
```{r proflike, eval = F}
